...
const errorHandler = (error, request, response, next) => {
  ...
  if (error.name === "PasswordValidationError") return response.status(400).send({ error: error.message });
  if (error.name === "JsonWebTokenError") return response.status(401).send({ error: error.message });
  if (error.name === "TokenExpiredError") return response.status(401).send({ error: error.message });
...
const userExtractor = (request, response, next) => {
  ...
  if (!decodedToken.id) return response.status(401).send({ error: "token invalid" });
